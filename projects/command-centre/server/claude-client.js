 ...
 * @param {Object} params
 * @param {string} params.model - 'opus', 'sonnet', or 'haiku'
 * @param {string} params.systemPrompt - System prompt for the agent
 * @param {string} params.userMessage - The task/message to send
 * @param {number} [params.maxTokens=1024] - Max response tokens
 * @param {Array} [params.tools] - Optional tools array (e.g. web_search)
 * @param {Array} [params.messages] - Optional full messages array (for multi-turn)
 * @returns {Object} { success, result, tokens, model, duration, searchCount } or { success: false, error }
 */
export async function executeCall({ model, systemPrompt, userMessage, maxTokens = 1024, tools, messages }) {
  ...
  try {
    ...
    // Extract text and tool usage from mixed content blocks
    const textParts = []
    let searchCount = 0
    ...
    for (const block of response.content) {
      if (block.type === 'text') {
        ...
        // Log tool usage (web search query etc.)
        console.log(`  [Tool] ${block.name}: ${JSON.stringify(block.input)}`)
      } else if (block.type === 'web_search_tool_result') {
        // Count search results for stats
        searchCount++
      }
    }

    // Also check usage for server tool use stats
...
/**
 * Streaming Claude API call — yields SSE-compatible events
 * @param {Object} params - Same as executeCall
 * @yields {Object} { type: 'text_delta'|'tool_use'|'done'|'error', ... }
 */
...
/**
 * Check which models are available by making a minimal call
...
export async function checkAvailableModels() {
  ...
  for (const [name, modelId] of Object.entries(MODEL_MAP)) {
    try {
      ...
      // Model not available or API error — skip
    }
  }
