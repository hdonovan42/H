...
function buildHTML(listing, current, total) {
  ...
  const dots = Array.from({ length: total }, (_, i) =>
    `<span style="display:inline-block;width:5px;height:5px;border-radius:50%;background:${i === current ? ACCENT : TEXT_DIM + '44'}"></span>`
  ).join('')
...
<div style="padding:6px 12px;display:flex;align-items:center;gap:10px;font-size:9px;border-bottom:1px solid ${ACCENT}1a">
  <span style="color:${ACCENT};font-weight:700;letter-spacing:2px">AUTOSNIPE</span>
  <span style="color:${ACCENT}44">\u2502</span>
  <span style="color:#f8f8ff;font-size:11px;font-weight:600;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${title.replace(/</g, '&lt;')}</span>
  ${price ? `<span style="color:${ACCENT};font-weight:700;font-size:11px">${price}</span>` : ''}
  <span style="color:${ACCENT}44">\u2502</span>
  <span id="close" style="color:${TEXT_DIM};cursor:pointer">[ CLOSE ]</span>
</div>
<div style="padding:5px 12px;display:flex;align-items:center;justify-content:space-between;font-size:9px;background:linear-gradient(180deg,${BG_DARK},#161620)">
...
export function openNavPopup(listing, current, total) {
  const left = screen.width - POPUP_WIDTH
  const top = 0
  const features = `width=${POPUP_WIDTH},height=${NAV_HEIGHT},top=${top},left=${left},menubar=no,toolbar=no,location=no,status=no,resizable=no,scrollbars=no`
